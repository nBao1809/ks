...
class ReportService:
    @staticmethod
    def revenue(period, year, month=None, quarter=None):
    ...
    def dashboard(target_date=None):
        ...
        from app.models import HousekeepingTask, TaskStatus
        return {
            'today_check_ins': Booking.objects.filter(check_in_date=target, status=BookingStatus.CONFIRMED).count(),
            'today_check_outs': Booking.objects.filter(check_out_date=target, status=BookingStatus.CHECKED_IN).count(),
            ...
            'today_revenue': str(
                Payment.objects.filter(paid_at__date=target, status=PaymentStatus.COMPLETED).aggregate(
                    t=Sum('amount'),
                )['t'] or 0,
            ),
